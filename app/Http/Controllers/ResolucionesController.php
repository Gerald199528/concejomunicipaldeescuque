...
use App\Models\Resolucion; // Cambiar a resolucion
...
class ResolucionesController extends Controller // Cambiar OrdenanzasController a ResolucionesController
{
    ...
    {
        $resoluciones = resolucion::all(); // Cambiar a resolucion
        return view('instrumentoslegales.resoluciones.index', compact('resoluciones')); // Cambiar a resolucion
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('resolucion')) { // Cambiar a resolucion
            $file = $request->file('resolucion');
            $fileName = $file->getClientOriginalName(); // Mantener el nombre original del archivo
            // Verificar si el archivo ya existe en la base de datos
            if (resolucion::where('nombre', $fileName)->exists()) { // Cambiar a resolucion
                return redirect()->back()->with('error', 'Estimado usuario el documento ya existe en la base de datos.');
            }
            $file->storeAs('public/resoluciones', $fileName); // Mantener la ruta de almacenamiento
            resolucion::create([
                'nombre' => $fileName,
                'ruta' => 'resoluciones/' . $fileName, // Mantener la ruta de almacenamiento
    ...
    {
        $filePath = 'public/resoluciones/' . $fileName; // Mantener la ruta de almacenamiento
        if (Storage::exists($filePath)) {
            Storage::delete($filePath);
            resolucion::where('nombre', $fileName)->delete(); // Cambiar a resolucion
